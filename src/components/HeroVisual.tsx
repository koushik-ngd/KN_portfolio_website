...
export default function HeroVisual() {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const dotR = Math.round(192 + (255 - 192) * colorT);
      const dotG = Math.round(132 + (255 - 132) * colorT);
      const dotB = Math.round(252 + (255 - 252) * colorT);
      // dot surface: purple-white rgb(233,213,255) → white
      const surfR = Math.round(233 + (255 - 233) * colorT);
      const surfG = Math.round(213 + (255 - 213) * colorT);
      const surfB = 255;
      ...
      for (const p of pp) {
        ...
        grd.addColorStop(0, `rgba(${dotR},${dotG},${dotB},${(alpha * 0.6).toFixed(3)})`);
        grd.addColorStop(1, `rgba(${dotR},${dotG},${dotB},0)`);
        ctx.beginPath();
        ...
        ctx.fillStyle = `rgba(${surfR},${surfG},${surfB},${alpha.toFixed(3)})`;
        ctx.fill();
      }
